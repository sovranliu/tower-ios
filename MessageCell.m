...
- (void)setCellFrame:(CellFrameModel *)cellFrame
{
    ...
    _timeLabel.text = [NSString stringWithFormat:@"%@",message.time];// message.time;
    
    _iconView.frame = cellFrame.iconFrame;
    NSString *iconStr = message.type ? self.docImgURL : @"user_photo_default";

    [_iconView setImageWithURL:[NSURL URLWithString:iconStr] placeholderImage:[UIImage imageNamed:@"user_photo_default"]];
    
//    _iconView.image = [UIImage imageNamed:iconStr];
    
    _textView.frame = cellFrame.textFrame;
    NSString *textBg = message.type ? @"askdoctor_chat_recive" : @"askdoctor_chat_send";
